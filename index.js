(function () {
    ...
    function applyDrawerState() {
        ...
        if (appState.drawerWidth) {
            drawer.style.setProperty('--drawer-width', appState.drawerWidth + 'px');
        }
    ...
    function buildTreeHelper(container, nodes, level) {
        nodes.forEach(node => {
            ...
            const stateExpanded = appState.openedIds.includes(node.id);

            // Collapsed if (not yet loaded) OR (not in opened list)
            if (lazyCollapsed || !stateExpanded) div.classList.add('tree-node--collapsed');
    ...
    async function toggleNode(div, node, btn, childCont, level) {
        ...
        if (isCollapsedNow) {
            ...
            if (!appState.openedIds.includes(node.id)) appState.openedIds.push(node.id);
            saveAppState();
            ...
            div.classList.add('tree-node--collapsed');
            btn.textContent = '▸';
            const idx = appState.openedIds.indexOf(node.id);
            if (idx >= 0) appState.openedIds.splice(idx, 1);
            saveAppState();
